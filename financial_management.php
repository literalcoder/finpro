...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($action === 'add') {
        ...
        $sql = "INSERT INTO financial_transactions (org_id, type, category, amount, description, created_by, transaction_date) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        if ($stmt->execute([$org_id, $type, $category, $amount, $description, $_SESSION['user_id'], $transaction_date])) {
...
if ($action === 'list') {
    ...
    // Calculate balance
    $sql = "SELECT COALESCE(SUM(CASE WHEN type IN ('income', 'collection') THEN amount ELSE -amount END), 0) as balance FROM financial_transactions WHERE org_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$org_id]);
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="text" name="category" class="form-control" placeholder="e.g., Membership Fees, Event Expenses" required>
              </div>
            </div>
          </div>
          <div class="row">
